-- --------------------------------------------------------------------------------
-- Name: Odjoutchone Sylvestre TOKOU 
...
-- Drop Procedure
IF OBJECT_ID ('uspSelectPilot')		            IS NOT NULL DROP  PROCEDURE uspSelectPilot
IF OBJECT_ID ('uspUpdatePilotProfile')		    IS NOT NULL DROP  PROCEDURE uspUpdatePilotProfile
IF OBJECT_ID ('uspSelectPilotFutureFlights')    IS NOT NULL DROP  PROCEDURE uspSelectPilotFutureFlights
IF OBJECT_ID ('uspAssignPilotToFlight')		    IS NOT NULL DROP  PROCEDURE uspAssignPilotToFlight
IF OBJECT_ID ('uspAddPilot')		            IS NOT NULL DROP  PROCEDURE uspAddPilot
IF OBJECT_ID ('uspSelectPilotPastMilesTotal')   IS NOT NULL DROP  PROCEDURE uspSelectPilotPastMilesTotal
...
(
	...
	,CONSTRAINT TPlanes_PlaneType_FK FOREIGN KEY ( intPlaneTypeID )
		REFERENCES TPlaneTypes ( intPlaneTypeID )
)
...
(
	...
	,strAirportCode		VARCHAR(10)		NOT NULL
...
(
	...
	,intStateID			INTEGER			    NOT NULL
	,strZip				VARCHAR(20)		    NOT NULL
	,strPhoneNumber		VARCHAR(20)		    NOT NULL
	,strEmail			VARCHAR(255)	    NOT NULL

	,CONSTRAINT TPassengers_PK PRIMARY KEY ( intPassengerID )

	,CONSTRAINT TPassengers_TStates_FK FOREIGN KEY ( intStateID )
		REFERENCES TStates ( intStateID )
)
...
(
	...
	,CONSTRAINT TPilots_TPilotRoles_FK FOREIGN KEY ( intPilotRoleID )
		REFERENCES TPilotRoles ( intPilotRoleID )
)
...
(
	...
	,intPlaneID				INTEGER			NOT NULL
	,intReservedSeats		INTEGER			NOT NULL DEFAULT 0

	,CONSTRAINT TFlights_PK PRIMARY KEY ( intFlightID )

	,CONSTRAINT TFlights_FromAirport_FK FOREIGN KEY ( intFromAirportID )
		REFERENCES TAirports ( intAirportID )

	,CONSTRAINT TFlights_ToAirport_FK FOREIGN KEY ( intToAirportID )
		REFERENCES TAirports ( intAirportID )

	,CONSTRAINT TFlights_Plane_FK FOREIGN KEY ( intPlaneID )
		REFERENCES TPlanes ( intPlaneID )
)
...
(
	...
	,CONSTRAINT TMaintenances_TPlanes_FK FOREIGN KEY ( intPlaneID )
		REFERENCES TPlanes ( intPlaneID )
)
...
(
	...
	,CONSTRAINT TPilotFlights_Pilot_FK FOREIGN KEY ( intPilotID )
		REFERENCES TPilots ( intPilotID )

	,CONSTRAINT TPilotFlights_Flight_FK FOREIGN KEY ( intFlightID )
...
(
	...
	,CONSTRAINT TAttendantFlights_Attendant_FK FOREIGN KEY ( intAttendantID )
		REFERENCES TAttendants ( intAttendantID )

	,CONSTRAINT TAttendantFlights_Flight_FK FOREIGN KEY ( intFlightID )
		REFERENCES TFlights ( intFlightID )
)
...
(
	...
	,intFlightID			INTEGER			NOT NULL
	,intPassengerID			INTEGER			NOT NULL
	,strSeat				VARCHAR(20)		NOT NULL
	,strSeatType			VARCHAR(20)		NOT NULL DEFAULT 'Reserved'

	,CONSTRAINT TFlightPassengers_PK PRIMARY KEY ( intFlightPassengerID )

	,CONSTRAINT TFlightPassengers_Flight_FK FOREIGN KEY ( intFlightID )
		REFERENCES TFlights ( intFlightID )

	,CONSTRAINT TFlightPassengers_Passenger_FK FOREIGN KEY ( intPassengerID )
		REFERENCES TPassengers ( intPassengerID )
)
...
(
	...
	,CONSTRAINT TMaintenanceMaintenanceWorkers_Maintenance_FK FOREIGN KEY ( intMaintenanceID )
		REFERENCES TMaintenances ( intMaintenanceID )

	,CONSTRAINT TMaintenanceMaintenanceWorkers_Worker_FK FOREIGN KEY ( intMaintenanceWorkerID )
		REFERENCES TMaintenanceWorkers ( intMaintenanceWorkerID )
)
...
VALUES				  (1, '4/1/2022', '111', '10:00:00', '12:00:00', 1, 2, 1200, 2)
					 ...
					 ,(3, '4/5/2026', '333','15:00:00', '17:00:00', 1, 5, 1200, 3)
					 ...
					 ,(9, '4/19/2027', '999','08:00:00', '10:00:00', 4, 2, 1000, 6)
...
INSERT INTO TUserLogins (intUserLoginID, strUserName, strPassword, strRole, intUserID)
VALUES
     (1,  'knelly',      'pass1',   'Passenger',  1)
    ,(2,  'orville',     'pass2',   'Passenger',  2)
    ,(3,  'eileen',      'pass3',   'Passenger',  3)
    ,(4,  'bob',         'pass4',   'Passenger',  4)
    ,(5,  'ware',        'pass5',   'Passenger',  5)
    ,(6,  'kay',         'pass6',   'Passenger',  6)

    ,(7,  'tip',         'pilot1',  'Pilot',     1)
    ,(8,  'ima',         'pilot2',  'Pilot',     2)
    ,(9,  'hugh',        'pilot3',  'Pilot',     3)
    ,(10, 'iwanna',      'pilot4',  'Pilot',     4)
    ,(11, 'rose',        'pilot5',  'Pilot',     5)

    ,(12, 'miller',      'att1',    'Attendant', 1)
    ,(13, 'sherley',     'att2',    'Attendant', 2)
    ,(14, 'buhh',        'att3',    'Attendant', 3)
    ,(15, 'myles',       'att4',    'Attendant', 4)
    ,(16, 'walker',      'att5',    'Attendant', 5)

    ,(17, 'admin',       'admin1',   'Admin',     0);

    
GO
CREATE PROCEDURE uspSelectPilot
   @PilotID INT
AS
BEGIN
    SELECT 
...
CREATE PROCEDURE uspUpdatePilotProfile
    @PilotID INT,
...
BEGIN
    SELECT 
        F.strFlightNumber,
        F.dtmFlightDate,
        A.strAirportCode AS FromAirport,
        B.strAirportCode AS ToAirport
    FROM TPilotFlights PF
...
CREATE PROCEDURE uspAssignPilotToFlight
    @PilotFlightID INT,
    @PilotID INT,
    @FlightID INT
AS
BEGIN
    INSERT INTO TPilotFlights
        (intPilotFlightID, intPilotID, intFlightID)
    VALUES
        (@PilotFlightID, @PilotID, @FlightID)
END
GO
...
BEGIN
    ...
    VALUES
        (@PilotID, @FirstName, @LastName, @EmployeeID,
         @HireDate, @TerminationDate, @LicenseDate, @PilotRoleID)
END
GO



CREATE PROCEDURE uspSelectPilotPastMilesTotal
    @PilotID INT
AS
BEGIN
